...
#' @description
#' \code{jd_regarima_specDefX13} creates (and modifies), from a predefined \emph{JDemetra+} model specification, a \code{c("JD_RegArima_Spec","X13")} class object with the RegARIMA model specification for the X13 method.
#'
...
#'
#' @examples
#'   myspec1 <-jd_regarima_specDefX13(spec=c("RG5c"))
#'   myspec2 <-jd_regarima_specDefX13(spec=c("RG5c"), tradingdays.option = "WorkingDays")
#'   myreg1 <-jd_regarima(myseries, spec=myspec1)
#'   myreg2 <-jd_regarima(myseries, spec=myspec2)
#'
#'
#' # Pre-specified outliers
#'   myspec1<-jd_regarima_specDefX13(spec=c("RG5c"),usrdef.outliersEnabled = TRUE,
...
#'
#'   myspec1 <- jd_regarima_specDefX13(spec="RG5c", usrdef.varEnabled = TRUE,
#'                                   usrdef.var = var)
#'   myreg1 <- jd_regarima(myseries,myspec1)
#'   myreg1
#'
#'   myspec2 <- jd_regarima_specDefX13(spec="RG5c", usrdef.varEnabled = TRUE,
...
#'   myspec1 <- jd_regarima_specDefX13(spec="RG5c", automdl.enabled =FALSE,
...
#'
#' @return
#' A list of class \code{c("JD_RegArima_Spec","X13")} with the below components. Each component refers to different part of the RegARIMA model specification, mirroring the arguments of the function (for details see arguments description). Each of the lowest-level component (except span, pre-specified outliers, user-defined variables and pre-specified ARMA coefficients) is structured within a data frame with columns denoting different variables of the model specification and rows referring to: first row - base specification, specification of the \code{c("JD_RegArima","X13")} or \code{c("JD_RegArima_Spec","X13")} object as provided within the argument \code{object}; second row - user modifications as specified by the remaining arguments of the function (e.g.: \code{arima.d}); and third row - final model specification, values that will be used in the function \code{\link{jd_regarima}}. The final specification (third row) shall include user modifications (row two) unless they were wrongly specified. The pre-specified outliers, user-defined variables and pre-specified ARMA coefficients consist of a list with the \code{Predefined} (base model specification) and \code{Final} values.
#'
#' \item{estimate}{data frame. Variables referring to: \code{span} - time span for the model estimation, \code{tolerance} - argument \code{estimate.tol}. The final values can be also accessed with the function \code{\link{s_estimate}}.}
#'
#' \item{transform}{data frame. Variables referring to: \code{tfunction} - argument \code{transform.function}, \code{adjust} - argument \code{transform.adjust}, \code{aicdiff} - argument \code{transform.aicdiff}. The final values can be also accessed with the function \code{\link{s_transform}}.}
...
#'   myspec<- jd_regarima_specX13(myreg)
#'   myreg1<- jd_regarima(myseries,myspec)
#'
#' # Modify the model specification from a "JD_RegArima" object
#'   myspec2 <- jd_regarima_specX13(myreg,tradingdays.option = "WorkingDays")
#'   myreg2 <- jd_regarima(myseries,myspec2)
#'
#' # Modify the model specification from a "JD_RegArima_Spec" object
#'   myspec3 <- jd_regarima_specX13(myspec,tradingdays.option = "WorkingDays")
...
#'   var <-ts(matrix(c(var1,var2), ncol=2),start = c(2001, 12), frequency = 12)
#'
#'   myspec1 <- jd_regarima_specX13(myreg, usrdef.varEnabled = TRUE,
#'                                   usrdef.var = var)
#'   myreg1 <- jd_regarima(myseries,myspec1)
#'   myreg1
#'
#'   myspec2 <- jd_regarima_specX13(myreg, usrdef.varEnabled = TRUE,
#'                                   usrdef.var = var1, usrdef.varCoef = c(2),
#'                                   transform.function = "None")
#'   myreg2 <- jd_regarima(myseries, myspec2)
#'   s_preVar(myreg2)
